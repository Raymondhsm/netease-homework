...
class NetStream(object):
	...
	# update
	def process(self):
		if self.state == config.NET_STATE_STOP:
			return 0
	...
	def send(self, data):
		size = len(data) + config.NET_HEAD_LENGTH_SIZE
		wsize = struct.pack(config.NET_HEAD_LENGTH_FORMAT, size)
	...
	def recv(self):
		...
		self.__recvRaw(config.NET_HEAD_LENGTH_SIZE)

		return self.__recvRaw(size - config.NET_HEAD_LENGTH_SIZE)
