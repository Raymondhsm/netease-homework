...
SkyboxRender::SkyboxRender(const std::shared_ptr<DX::DeviceResources>& deviceResources, const std::shared_ptr<Camera>& camera):
	m_indexCount(0),
...
{
	// 加载顶点和像素着色器
	auto loadVSTask = DX::ReadDataAsync(L"SkyboxVS.cso");
	auto loadPSTask = DX::ReadDataAsync(L"SkyboxPS.cso");
...
void SkyboxRender::Update(DX::StepTimer const & timer)
{
	XMStoreFloat4x4(&m_constantBufferData.view, XMMatrixTranspose(m_camera->GetView()));
